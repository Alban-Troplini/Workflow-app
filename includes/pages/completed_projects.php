...
<?php
  ...
  //pagination!
      ...
      $project_query_count = "SELECT COUNT(*) as pro_count FROM proejcts WHERE project_status='Completed'";
    ...
    $sql_completed = "SELECT * FROM proejcts WHERE ";
    $sql_completed .= " project_status='Completed' ";
    $sql_completed .= " ORDER BY project_id DESC LIMIT {$page_1}, {$per_page}";

  // show all projects
      $result_all = $connect->query($sql_completed);
    ...
    <div class="col-sm-4">
      ...
      <div class="col-md-6 row">
           ...
           <form action="" method="POST">
                  ...
                  <button class="btn btn-warning float-right" onclick="return confirm('Are you sure to send on WORK?')" name="working_project" type="submit">Back to Working</button>
          </form></div>
           <div class="col-sm">
    ...
    <hr>
        ...
        for($i=1; $i <= $count_projects; $i++ ){
          ...
          if($i == $page){
            ...
            echo "<li class='page-item'><a class='active_link page-link' href='index.php?ins=completed_projects.php&ul={$i}'>{$i}</a></li>";

          } else {

           echo "<li class='page-item'><a class='page-link' href='index.php?ins=completed_projects.php&ul={$i}'>{$i}</a></li>";
        }
      }
        ?>
        </ul>
